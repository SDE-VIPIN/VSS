...
/**
 * Servlet implementation class Picked_Booking
 */
@WebServlet("/Picked_Booking")
public class Picked_Booking extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Picked_Booking() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int status = dao.PickedBooking(bookingid);
		if(status>0) {
			request.setAttribute("msg", userMessage.SERVICE_MESSSAGE);
			RequestDispatcher rd =request.getRequestDispatcher("/provider/view_dropped_vehicles.jsp"); //relative path
